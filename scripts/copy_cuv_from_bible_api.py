#!/usr/bin/env python3
"""
Copy CUV data from bible_api/cmn_cuv and convert to app format
"""
...
def process_cuv_files():
    """Copy and convert CUV files from bible_api to Resources/BibleData/cuv"""
    script_dir = Path(__file__).parent
    project_root = script_dir.parent
    
    source_dir = project_root / "bible_api" / "cmn_cuv"
    target_dir = project_root / "livingdevotional" / "Resources" / "BibleData" / "cuv"
    
    if not source_dir.exists():
        print(f"❌ Source directory not found: {source_dir}")
        return
    
    print(f"📖 Copying CUV files from: {source_dir}")
