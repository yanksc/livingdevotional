#!/usr/bin/env python3
"""
Copy KJV (King James Version) data from bible_api/eng_kjv 
...
def process_kjv_files():
    """Copy and convert KJV files from bible_api to BibleData.bundle/kjv"""
    script_dir = Path(__file__).parent
    project_root = script_dir.parent
    
    source_dir = project_root.parent / "bible_api" / "eng_kjv"
    
    # Target location: livingdevotional/Resources/BibleData.bundle/kjv
    target_bundle = project_root / "livingdevotional" / "Resources" / "BibleData.bundle"
    target_dir = target_bundle / "kjv"
    ...
    if not target_bundle.exists():
        print(f"❌ Target bundle not found: {target_bundle}")
        print(f"   Please ensure BibleData.bundle exists in the project")
        return
    
    print(f"📖 Copying KJV files from: {source_dir}")
    ...
    print(f"\n📝 Next steps:")
    print(f"   1. Verify the kjv folder is inside BibleData.bundle")
    print(f"   2. Update Models.swift to add KJV support")
    print(f"   3. Update BibleService.swift to load KJV data")
    print(f"   4. Build and test the app")
